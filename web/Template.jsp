<%-- 
    Document   : Template
    Created on : Jul 17, 2019, 9:50:25 PM
    Author     : hp
--%>

<%@page contentType="text/html" pageEncoding="UTF-8"%>
<!DOCTYPE html>

<table border="0" bgcolor='#000080' align='top' width='100%' style="height:100px">
    <tr>
        <td bgcolor='#000080' align='center'>
        <font style="font-family: 'Arial Rounded MT Bold', Gadget, sans-serif; size:"4" color='#FFE4B5">EXOTICA TRAVELS</font>
        </td>
        <td bgcolor="#000080" align='left' width='150'>
            <img src="Images/sidebar2.jpg.png" width="150" height="100" 
                 align='right'/>
        </td>
    </tr>
</table>
<table bgcolor='white' border='0' align='top' width='100%' style="height:50px">
    <tr>
        <td style="width: 216px;" colspan="1" rowspan="4">
            <img style="width: 150px; height: 100px;" alt="" src='Images/sidebar.jpg'>
        </td>
            
    
</table>